...
void test_drawDblMatchPoints(double alpha,int winn,int winp,char * fic1,char* fic2,char * fic_s){
  image im1(fic1);
  image im2(fic2);

  image* sortie=im1.drawDblMatchPoints(im2,50,winn,winp,&image::zncc,true);
  sortie->EcrireImagePGM(fic_s);
}
